...
// Plugin-only presets. Codex renders these in a single line; ordering is
// preserved as written. Layout reads left-to-right by priority:
// budget axis (context + tokens + limits) → identity (model) → place (branch +
// cwd) → state. Items that overlap with model-with-reasoning (e.g. fast-mode,
// which already shows up as " fast" suffix on the model item) are deliberately
...
Usage:
  ...
  codex-hub configure [--preset full|cockpit|essential|minimal] [--items a,b,c] [--add a,b] [--remove c] [--dry-run] [--force]
