...
Commands:
  ...
  uninstall   Remove codex-hub.exe and restore the Codex npm launcher backup.

Default Codex source path on Windows:
  C:\\src\\codex-source
`);
}
  ...
  while (stack.length > 0) {
    ...
    for (const entry of fs.readdirSync(current, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        stack.push(fullPath);
      } else if (entry.name === "codex-hub.exe") {
        return fullPath;
      }
    }
  }
...
function assertWindows(command) {
  if (process.platform !== "win32") {
    throw new Error(`${command} is currently Windows-only`);
  }
}
